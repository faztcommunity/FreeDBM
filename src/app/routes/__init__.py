...
class Responds():
    ...
    def validate_json_signup(cls, json_file):
        v = Validator()
        return v.validate(json_file, JSON.load_json('user'))

    @classmethod
    # TODO(jsgonzlez661): Validate schema json for login
    def validate_json_login(cls, json_file):
        v = Validator()
        return v.validate(json_file, JSON.load_json('login'))

    @classmethod
    # TODO(jsgonzlez661): Validate schema json for get new token
    def validate_json_token(cls, json_file):
        v = Validator()
        return v.validate(json_file, JSON.load_json('user_token'))
